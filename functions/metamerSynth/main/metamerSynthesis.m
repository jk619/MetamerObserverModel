...
for niter=1:opts.nIters
  ...
  if opts.debugging
    ...
    tmpVerbose      = opts.verbose;
    opts.verbose    = 0; 
    [tmpparams]     = metamerAnalysis(oim.full,m,opts);
    opts.verbose    = tmpVerbose;
    
    % compare all statistics to desired ones
    for iw = 1:m.scale{1}.nMasks
      statg0tmp = tmpparams.pixelStats(:,iw);
    ...
    for nsc=Nsc+1:-1:1
      for iw = 1:m.scale{nsc}.nMasks
        tmpSignal = tmpparams.autoCorrReal.scale{nsc}.mask{iw};
    ...
    for nsc=Nsc:-1:1
      for nor=1:Nor
        for iw=1:m.scale{nsc}.nMasks
          tmpSignal = tmpparams.autoCorrMag.scale{nsc}.ori{nor}.mask{iw};
    ...
    for nsc=Nsc:-1:1
      for iw = 1:m.scale{nsc}.nMasks
        tmpSignal = tmpparams.cousinMagCorr.scale{nsc}.mask{iw};
    ...
    for nsc=Nsc-1:-1:1
      for iw = 1:m.scale{nsc}.nMasks
        tmpSignal = tmpparams.parentMagCorr.scale{nsc}.mask{iw};
    ...
    for nsc=Nsc:-1:1
      for iw=1:m.scale{nsc}.nMasks
        tmpSignal = tmpparams.parentRealCorr.scale{nsc}.mask{iw};
    ...
    for nsc=Nsc:-1:1
      for iw=1:m.scale{nsc}.nMasks
        tmpSignal = tmpparams.cousinRealCorr.scale{nsc}.mask{iw};
    ...
    for iband=1:size(oimpind,1);
      for iw=1:size(tmpparams.magMeans.band{iband},1)
        tmpSignal = tmpparams.magMeans.band{iband}(iw);

function [sampPerf_L1,sampPerf_L2,sampPerf_L4,out] = getPerf2afc_window(allResp,inParams,iP)
%%
...
%             trials_L1  = nan*zeros(inParams.nTrials,2,length(inParams.groupInds));
            trials_L2  = nan*zeros(inParams.nTrials,length(inParams.groupInds));
%             trials_L4  = nan*zeros(inParams.nTrials,2,length(inParams.groupInds));
            ...
            for iTrial=1:inParams.nTrials
                ...
                [~,~,outS_L2,outN_L2,~,~] = getFullResp(tmpTexA,tmpTexA,tmpTexB,tmpNoise(:,:,:,iTrial),inParams);
                
%                 trials_L1(iTrial,1)     = outN_L1;
%                 trials_L1(iTrial,2)     = outS_L1;
%                 
%                 trials_L2(iTrial,1,:)     = outN_L2;
                trials_L2(iTrial,:)     = outS_L2;
%                 
...
%             
            [~,b] = max(trials_L2,[],2);      % find the interval with minimum distanc
            sampPerf_L2(iT,iEcc,iS,:)   = b;
...
if ~inParams.singlePool
    ...
    if    inParams.broderickTask
        ...
        signalParams_L1    = sum((abs((tmpTexANoise  - tmpTexBNoise)')));
        signalParams_L2    = sqrt(sum(((tmpTexANoise  - tmpTexBNoise)').^2));
        signalParams_L4    = sqrt(sum(((tmpTexANoise  - tmpTexBNoise)').^4));
        
    
        noiseParams_L1     = sum((abs((tmpTexANoiseB  - tmpZeroANoise)')));
        noiseParams_L2     = sqrt(sum(((tmpTexANoiseB  - tmpZeroANoise)').^2));
        noiseParams_L4     = sqrt(sum(((tmpTexANoiseB  - tmpZeroANoise)').^4));

...
%               5) Correlation of magnitudes (or)
%               6) Correlation of magnitudes (sc)
%               7) Correlation of magnitudes (sp)
...
% 
% figure(imf);   
% clf;showIm(im,'auto',1); title(sprintf('iteration 0')); drawnow;
...
%% MAIN LOOP
% disppercent(-inf,'synthesisizing texture');
for niter = 1:Niter
% disppercent(niter/Niter);

%tmp = im-mean(im(:));
%tmp = tmp/std(tmp(:));
%tmp = tmp*63 + 255/2;
%imwrite(clip(tmp,0,255)/255,sprintf('iter-%g.png',niter));
...
if cmask(3),
    ...
    if cmask(5) & ~cmask(6)
      if (np == 0)
        ...
        [cousins] = adjustCorr1s(cousins, C0(1:nc,1:nc,nsc), 2, p);
      end
    end
    if ~cmask(5) & cmask(6)
      if (np == 0)
        %[cousins, snr3(niter,nsc)] = adjustCorr1s(cousins, innerProd(cousins)/cousinSz, 2, p);
      else
        for icous = 1:size(cousins,2)
          tmp = cousins(:,icous);
          cousins(:,icous) = adjustCorr2s(tmp,mean(tmp.^2),parents,Cx0(icous,1:np,nsc), 3, p);
        end
        %cousins = adjustCorr2s(cousins, C0(1:nc,1:nc,nsc), parents, Cx0(1:nc,1:np,nsc), 3, p);
      end
    end
    ...
    %% Adjust autoCorr of mag responses 
      ...
      for nor = 1:Nor,
        ...
        [ch, snr1(niter,nband-1)] = modacor22(ch,...
          ...
          vari =  ace0(la+1:la+1,la+1:la+1,nsc,nor);
          ch = ch*sqrt(vari/var2(ch));
        end
        ch = real(ch);
...
commented = 0;  % set it to 0 for displaying convergence of parameters in SNR (dB)
if ~commented,    

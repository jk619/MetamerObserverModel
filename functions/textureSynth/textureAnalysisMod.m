function [params] = textureAnalysisMod(im0, Nsc, Nor, Na)
...
%
%
%  modified by jeremy freeman feb 11, 2010 to compute covariance among
%    phase-doubled parents (real and imaginary, 'withinParentRealCorr')
%    so that full covariance matrices can be built for averaging log-cov.
%    this field is ignored in the synthesis code (which was no modified)
...
for nsc = 1:Nsc,
  ...
  helpCr0(1:nrc,1:nrc,nsc) = innerProd(cousins)/cousinSz;
  if (nrp > 0)
    
    Crx0(1:nrc,1:nrp,nsc) = (cousins'*rparents)/cousinSz;
    Crp0(1:nrp,1:nrp,nsc) = (rparents'*rparents)/cousinSz;
    if (nsc==Nsc)

function [im,snrP,imS] = texColorSynth3(params, im0, Niter, cmask, imask, pmask)

% res = texColorSynth3(params, initialIm, Niter, cmask, imask, pmask)
%
% Synthesize texture applying Portilla-Simoncelli model/algorithm,
% params: structure containing texture parameters (as returned by texColorAnlz3).
%
...
%		5) Display while processing (1 = on/ 0 = off)
%
% imask (optional): imsizex4 matrix.  First column is a binary mask, second,
...
snr1 = zeros(Niter,Nsc*Nor,Nclr);
snr2 = zeros(Niter,Nsc+1,Nclr);
snr3 = zeros(Niter,Nsc);
snr3r = zeros(Niter,Nsc);
snr4 = zeros(Niter,Nsc-1);
snr4r = zeros(Niter,Nsc-1);
snr6 = zeros(Niter,Nclr);
snr7 = zeros(Niter,2*(Nsc+1)+3,Nclr);
...
for niter = 1:Niter
    ...
    %imwrite(clip(im/255,0,255),sprintf('iter-%g.png',niter));
...
if 1,
  %% Adjust (try) spatial-color correlation in the LPR
...
if 1,
    if  cmask(4) & (Nrp ~= 0)
        ...
        for nrc = 1:Nrc,
          rcou = rcousins(:,nrc);
          [rcou, s3, s4] = ...
            adjustCorr2s(rcou,mean(rcou.^2),rparents,Crx0(nrc,1:Nrp,nsc), 3);
          a4 = s4 + a4;
          rcousins(:,nrc) = rcou;
        end
...
%% This does not work in this model!
%%    if (~isempty(pmaskInd))
%%        pyr(pmaskInd(:,clr),clr) = pmask(pmaskInd(:,clr), 2);
%%    end
...
if cmask(5),
  figure(snrf);
if cmask(1)
  subplot(181); plot(mean(snr7(1:niter,:,:),3)); title('Mrgl stats');
end
if cmask(2),
  subplot(182); plot(mean(snr2(1:niter,:,:),3)); title('Raw auto');
  subplot(183); plot(snr3r(1:niter,:)); title('Raw ori');
end
if cmask(3),
  subplot(184); plot(mean(snr1(1:niter,:,:),3)); title('Mag auto');
  subplot(185); plot(snr3(1:niter,:)); title('Mag ori');
  subplot(186); plot(snr4(1:niter,:)); title('Mag scale');
end
if (Nrp > 0) & cmask(4),
  subplot(187); plot(snr4r(1:niter,:)); title('Phs scale');
end
  subplot(188); plot(snr6(1:niter,:)); title('Im change');
  drawnow
end % cmask(5)

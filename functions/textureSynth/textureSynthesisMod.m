function [im,snrP,imS] = textureSynthesisMod(params, im0, Niter, cmask, oim)
...
% want cmask to work differently

% 1) marginals
% 2) auto correlations (both real and magnitudes)
% 3) cross correlations (across orientations and scale, magnitude only)

% will generate images for 1+2, and 1+3
...
for niter = 1:Niter
    ...
    % separately computely SNR for all parameters, incase any
    % are missing from the above steps (because they have been ignored)
    
    SNR = 0;
    
    if SNR
        if exist('oim','var');
            origParams = textureAnalysisMod(oim,Nsc,Nor,7);
        else
            origParams = params;
        end
        tmpParams = textureAnalysisMod(im,Nsc,Nor,7);
        snrAll = snrAllParams(origParams,tmpParams,snrAll,niter);
        plotSNR(snrAll,60,Niter); drawnow;
    end

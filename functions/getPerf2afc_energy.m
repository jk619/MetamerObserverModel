function [sampPerf_L1,sampPerf_L2,sampPerf_L4,out] = getPerf2afc_energy(allResp,inParams,iP)
%%
...
%%
if inParams.normBroderick

    dims            = [size(allResp{1},1) size(allResp{1},2) size(allResp{1},3)];
    normParamsSaveFile  = fullfile(loadPath, sprintf('cell_responses.mat'));
    normParamsSaveFile  = fullfile(loadPath, sprintf('cell_responses.mat'));
    load(normParamsSaveFile);

    for i = 1 : length(allResp)
        myresp = allResp{i};
        field = fieldnames(myresp);

        for f = 1 : length(field)

            mymean = mean(eval(field{f}));
            mystd  = std(eval(field{f}));

            myresp.(field{f}) = myresp.(field{f}) - mymean;
            myresp.(field{f}) = myresp.(field{f}) ./ mystd;
        end

        myrespreshape = zeros(length(field),length(myresp.lum));

        for f = 1 : length(field)
            myrespreshape(f,:) = myresp.(field{f});
        end
        allRespNBrod{1}(1,1,i,:,:) = myrespreshape;

    end
    allRespN = allRespNBrod;
...
for iEcc = 1:numel(allRespN)
    for iS = 1:size(allRespN{iEcc},1)
        for iT = 1:size(allRespN{iEcc},2)
            ...
            magInds                 = inParams.groupInds;
            tmpMagParams            = squeeze(allRespN{iEcc}(iS,iT,:,:,:));
...
%             trials  = trials + inParams.lateN*randn(size(trials));
            ...
            % get performance (correct is always first 0 distance interval)
